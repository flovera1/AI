...
/*
* This are multidimensional arrays, by multiplying its indices we can have a simple array.
* So, in this case the bidimensional array will be transformed in a simple array with the
* length remenbered by the compiler automatically.
*/
...
char *FindS(char *hypothesis,char *example,char *target){
	...
	if(consistent(target, example)){
		...
		for(i=0; i<6; i++){
			if(strcmp(hattr[i], "0") == 0) 
				strcpy(hattr[i], attr[i]);
			else if(strcmp(hattr[i], attr[i])) 
				strcpy(hattr[i], "?");
		}
